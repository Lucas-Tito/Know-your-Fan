...
export const verifyIdentity = async (userId, documentFile, selfieFile) => {
  try {
    ...
    const response = await fetch('https://know-your-fan-production-c1f0.up.railway.app/verify-identity', {
      method: 'POST',
      body: formData,
    });
...
export const addEsportsProfile = async (userId, profileData) => {
  try {
    const response = await fetch(`https://know-your-fan-production-c1f0.up.railway.app/users/${userId}/esports-profiles`, {
      method: 'POST',
      headers: {
...
export const validateEsportsProfile = async (profileUrl, userId = null) => {
  try {
    const url = userId 
      ? `https://know-your-fan-production-c1f0.up.railway.app/users/${userId}/validate_esports_profile_relevance`
      : 'https://know-your-fan-production-c1f0.up.railway.app/validate-esports-profile';

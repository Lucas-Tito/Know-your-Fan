...
export const linkBlueskyAccount = async (userId, credentials) => {
  try {
    const response = await fetch(`/users/${userId}/bluesky`, {
      method: 'POST',
      headers: {
...
export const updateBlueskyAccount = async (userId) => {
  try {
    const response = await fetch(`/users/${userId}/bluesky/update`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      }
...
export const unlinkSocialAccount = async (userId, platform) => {
  try {
    const response = await fetch(`/users/${userId}/social-accounts/${platform}`, {
      method: 'DELETE'
    });
...
export const getEsportsActivity = async (userId) => {
  try {
    const response = await fetch(`/users/${userId}/esports-activity`);
